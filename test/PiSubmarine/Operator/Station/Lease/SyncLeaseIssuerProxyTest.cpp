...
namespace PiSubmarine::Operator::Station::Lease
{
    ...
    TEST(SyncLeaseIssuerProxyTest, RenewLeasePipelinesNextAsyncRenewalAfterCompletedResult)
    {
        AsyncLeaseIssuerStub asyncLeaseIssuer;
        SyncLeaseIssuerProxy proxy(asyncLeaseIssuer);
        const auto request = ::PiSubmarine::Lease::Api::LeaseRequest{
    ...
    TEST(SyncLeaseIssuerProxyTest, RenewLeaseFailureInvalidatesCachedGrant)
    {
        AsyncLeaseIssuerStub asyncLeaseIssuer;
        SyncLeaseIssuerProxy proxy(asyncLeaseIssuer);
        const auto request = ::PiSubmarine::Lease::Api::LeaseRequest{
        ...
        const auto reacquireResult = proxy.AcquireLease(request);
        ASSERT_FALSE(reacquireResult.has_value());
        EXPECT_EQ(reacquireResult.error().Condition, NotReadyCondition);
        EXPECT_EQ(asyncLeaseIssuer.AcquireEnqueueCount, 2);
    }
}
